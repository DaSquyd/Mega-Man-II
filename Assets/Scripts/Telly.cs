...
public class Telly : MonoBehaviour {
    ...
    Vector2 GetAngle() {
        ...
        float angle = Mathf.RoundToInt(Mathf.Atan2((player.transform.position.y + 0.77f) - transform.position.y, player.transform.position.x - transform.position.x) * Mathf.Rad2Deg / 45f) * 45 * Mathf.Deg2Rad;
